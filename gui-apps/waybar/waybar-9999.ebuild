...
LICENSE="MIT"
SLOT="0"
IUSE="mpd network +popups pulseaudio sndio tray +udev wifi libcxx experimental hyprland"
...
src_configure() {
	...
	if use hyprland; then
		sed -i 's/zext_workspace_handle_v1_activate(workspace_handle_);/const std::string command = "hyprctl dispatch workspace " + name_;\n\tsystem(command.c_str());/g' "${WORKDIR}/${P}/src/modules/wlr/workspace_manager.cpp" || die "Failed to enable hyprland support"
		echo "Building with hyprland support"
	fi
